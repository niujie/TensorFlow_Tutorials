...
def filter_max_length(x_, y_, max_length=MAX_LENGTH):
    return tf.logical_and(tf.size(x_) <= max_length,
                          tf.size(y_) <= max_length)
...
def positional_encoding(position, d_model_):
    ...
    pos_encoding_ = angle_rads[np.newaxis, ...]

    return tf.cast(pos_encoding_, dtype=tf.float32)
...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    output_ = tf.matmul(attention_weights, v)  # (..., seq_len_q, depth_v)

    return output_, attention_weights
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask):
        ...
        output_ = self.dense(concat_attention)  # (batch_size, seq_len_q, d_model)

        return output_, attention_weights


temp_mha = MultiHeadAttention(d_model_=512, num_heads_=8)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x_, training, mask):
        attn_output, _ = self.mha(x_, x_, x_, mask)  # (batch_size, input_seq_len, d_model)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x_, enc_output, training,
             look_ahead_mask, padding_mask):
        # enc_output.shape == (batch_size, input_seq_len, d_model)

        attn1, attn_weights_block1 = self.mha1(x_, x_, x_, look_ahead_mask)  # (batch_size, target_seq_len, d_model)
        attn1 = self.dropout1(attn1, training=training)
        out1 = self.layernorm1(attn1 + x_)
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers_, d_model_, num_heads_, dff_, input_vocab_size_,
        ...
        self.embedding = tf.keras.layers.Embedding(input_vocab_size_, d_model_)
    ...
    def call(self, x_, training, mask):
        ...
        for i in range(self.num_layers):
            x_ = self.enc_layers[i](x_, training, mask)

        return x_  # (batch_size, input_seq_len, d_model)


sample_encoder = Encoder(num_layers_=2, d_model_=512, num_heads_=8,
                         dff_=2048, input_vocab_size_=8500,
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, num_layers_, d_model_, num_heads_, dff_, target_vocab_size_,
                 maximum_position_encoding, rate=0.1):
        ...
        self.dec_layers = [DecoderLayer(d_model_, num_heads_, dff_, rate)
                           for _ in range(num_layers_)]
        self.dropout = tf.keras.layers.Dropout(rate)
    ...
    def call(self, x_, enc_output, training,
        ...
        for i in range(self.num_layers):
            x_, block1, block2 = self.dec_layers[i](x_, enc_output, training,
                                                    look_ahead_mask, padding_mask)
...
sample_decoder = Decoder(num_layers_=2, d_model_=512, num_heads_=8,
                         dff_=2048, target_vocab_size_=8000,
...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers_, d_model_, num_heads_, dff_, input_vocab_size_,
                 target_vocab_size_, pe_input, pe_target, rate=0.1):
        super(Transformer, self).__init__()

        self.encoder = Encoder(num_layers_, d_model_, num_heads_, dff_,
                               input_vocab_size_, pe_input, rate)

        self.decoder = Decoder(num_layers_, d_model_, num_heads_, dff_,
                               target_vocab_size_, pe_target, rate)

        self.final_layer = tf.keras.layers.Dense(target_vocab_size_)

    def call(self, inp_, tar_, training, enc_padding_mask,
             look_ahead_mask, dec_padding_mask):
        enc_output = self.encoder(inp_, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)

        # dec_output.shape == (batch_size, tar_seq_len, d_model)
        dec_output, attention_weights = self.decoder(
            tar_, enc_output, training, look_ahead_mask, dec_padding_mask)
...
sample_transformer = Transformer(
    num_layers_=2, d_model_=512, num_heads_=8, dff_=2048,
    input_vocab_size_=8500, target_vocab_size_=8000,
...
def train_step(inp_, tar_):
    ...
    with tf.GradientTape() as tape:
        predictions, _ = transformer(inp_, tar_inp,
                                     True,
                                     enc_padding_mask,
                                     combined_mask,
                                     dec_padding_mask)
...
def evaluate(inp_sentence):
    ...
    output_ = tf.expand_dims(decoder_input, 0)

    for i in range(MAX_LENGTH):
        enc_padding_mask, combined_mask, dec_padding_mask = create_masks(
            encoder_input, output_)
        ...
        if predicted_id == tokenizer_en.vocab_size + 1:
            return tf.squeeze(output_, axis=0), attention_weights

        # concatenate the predicted_id to the output which is given to the decoder
        # as its input.
        output_ = tf.concat([output_, predicted_id], axis=-1)
